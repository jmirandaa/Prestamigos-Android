...
public class NuevoAmigoComando extends Comando implements Callback<RespuestaREST<Long>> {
    public NuevoAmigoComando(int pantalla) {
        super(pantalla);
    }
    ...
    public void ejecutar(Object... args) {
        try {
            ...
            if (args.length != 2)
            {
                throw new RuntimeException("El argumento 1 debe ser el email destino y el argumento 2 el email origen");
            }
            String emailDestino = (String) args[0];
            ...
            Call<RespuestaREST<Long>> call = usuariosService.nuevoAmigo(emailDestino,emailOrigen);
            call.enqueue(this);
        }
